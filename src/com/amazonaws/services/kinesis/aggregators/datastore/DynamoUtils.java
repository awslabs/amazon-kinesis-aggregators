...
public class DynamoUtils {
     ...
     * @throws Exception
     */
    public static CreateTableResult safeCreateTable(final AmazonDynamoDB dynamoClient,
     ...
     * @throws Exception
     */
    public static DescribeTableResult safeDescribeTable(final AmazonDynamoDB dynamoClient,
     ...
     * 
     * @param dynamoClient Dynamo DB Client to use for connection to Dynamo DB.
     * @param dynamoTable The Table in Dynamo.
     * @throws Exception
     */
     ...
     * 
     * @param dynamoClient Dynamo DB Client to use for connection to Dynamo DB.
     * @param dynamoTable The table name to check.
    ...
    private static void waitForTableState(final AmazonDynamoDB dynamoClient,
        ...
        while (true) {
            try {
                ...
                if (tableRequest.getTable().getTableStatus().equals(status.name()))
                    break;
    ...
    public static void dropTable(final AmazonDynamoDB dynamoClient, final String dynamoTable)
            throws Exception {
        if (dynamoTable != null) {
            LOG.info(String.format("Dropping Dynamo Table %s", dynamoTable));
            try {
                dynamoClient.deleteTable(dynamoTable);
                waitForTableState(dynamoClient, dynamoTable, TableStatus.DELETING);
            } catch (ResourceNotFoundException e) {
    ...
    public static void cleanupAggTable(AWSCredentialsProvider credentials, Region region,
            final String dynamoTable, final String toSeq) throws Exception {
        ...
        final AmazonDynamoDB dynamoClient = new AmazonDynamoDBClient(credentials);
        if (region != null)
            dynamoClient.setRegion(region);
        final AmazonDynamoDBAsyncClient deleteCli = new AmazonDynamoDBAsyncClient(credentials);
        ...
        List<KeySchemaElement> keySchema = dynamoClient.describeTable(dynamoTable).getTable().getKeySchema();
        ...
        LOG.info(String.format("Deleting data from %s where %s values are below %s", dynamoTable,
                StreamAggregator.LAST_WRITE_SEQ, deleteBelow));
        int deleteCount = 0;

        do {
            // read data from the table
            ScanRequest scan = new ScanRequest().withTableName(dynamoTable).withAttributesToGet(
                    keyColumn, dateColumn, StreamAggregator.LAST_WRITE_SEQ).withExclusiveStartKey(
                    lastKey);
            ...
            for (Map<String, AttributeValue> map : results.getItems()) {
                ...
                if (Double.parseDouble(map.get(StreamAggregator.LAST_WRITE_SEQ).getS()) < deleteBelow) {
        ...
        LOG.info(String.format("Operation Complete - %s Records removed from Aggregate Store",
                deleteCount));
    }

    public static UpdateItemResult updateWithRetries(AmazonDynamoDB dynamoClient,
            UpdateItemRequest req) throws Exception {
        ...
        for (int i = 0; i < updateRetries; i++) {
            try {
                ...
                LOG.warn(String.format("Exceeded Provisioned Througput - Backing off for %s",
                        backoff));
                try {
     ...
     * @throws Exception
     */
    public static String getDynamoTableStructure(AmazonDynamoDB dynamoClient, String dynamoTable)
            throws Exception {
     ...
     * @throws Exception
     */
    public static List<String> getDictionaryEntry(final AmazonDynamoDB dynamoClient,
            final String dynamoTable) throws Exception {
        // get a list of all columns in the table, with keys first
        List<String> columns = new ArrayList<>();
        List<KeySchemaElement> keys = dynamoClient.describeTable(dynamoTable).getTable().getKeySchema();
        for (KeySchemaElement key : keys) {
            columns.add(key.getAttributeName());
        }
        ScanResult scan = dynamoClient.scan(new ScanRequest().withTableName(dynamoTable).withSelect(
    ...
    public static List<Map<String, AttributeValue>> queryUntilDone(AmazonDynamoDB dynamoClient,
            QueryRequest qr, int backoffMillis) throws Exception {
        ...
        do {
            ...
            do {
                try {
                    result = dynamoClient.query(qr).withLastEvaluatedKey(lastKeyEvaluated);
            ...
            if (result == null) {
                throw new Exception(String.format("Unable to execute Query after %s attempts",
                        queryAttempts));
            }
